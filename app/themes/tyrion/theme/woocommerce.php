...
function yit_shop_rating() {
    ...
    if ( yit_get_option( 'shop-view-show-rating' ) )  echo '<div class="classic-rating">' . $product->get_rating_html() . '</div>';
}
...
function yit_product_close_quick_view() {
    woocommerce_get_template( 'single-product/close-quick-view.php' );
}
...
function woocommerce_template_loop_product_thumbnail() {
    ...
    if ( $can_show_quick_view && yit_get_option('shop-use-quick-view') && $active_hover ) echo $quick_view;
    echo '</span>';
...
function yit_size_images_style() {
	...
	?>
	<style type="text/css">
        ul.products li.product.list .product-wrapper { padding-left:<?php echo yit_shop_catalog_w() + 30 + 7 + 2; ?>px; min-height:<?php echo yit_shop_catalog_h() + 7 + 2; ?>px; }
        ul.products li.product.list .product-wrapper .thumb-wrapper { margin-left:-<?php echo yit_shop_catalog_w() + 30 + 7 + 2; ?>px; width: <?php echo yit_shop_catalog_w()+10 ?>px; height: <?php echo yit_shop_catalog_h()+8 ?>px;  }
        ul.products li.product.list .product-wrapper img.onsale { left:<?php echo yit_shop_catalog_w() + 3 - 63; ?>px; }
        .og-expander.woocommerce div.product div.images div.thumbnails { height: <?php echo yit_shop_thumbnail_h() + 10 ?>px; }
        .single-product.woocommerce div.product.product-layout-1 div.images { width:<?php echo $images_container_w ?>%; }
	    .single-product.woocommerce div.product.product-layout-1 div.summary { width:<?php echo 100 - $images_container_w - $margin ?>%; padding-bottom:<?php echo yit_shop_thumbnail_h() ?>px;  }
	</style>
    <?php
}
...
function yit_add_to_cart_success_ajax( $datas ) {
    ...
    <p><i></i><?php _e( 'You added', 'yit' ) ?> <strong>"<?php echo get_the_title( $_REQUEST['product_id'] ) ?>"</strong> <?php _e( 'to your cart', 'yit' ) ?></p>
...
function yit_woocommerce_loginfobox(){
    ...
    echo    "<div class='row-fluid'><div class='log-info-container'>
                ...
                "<a href='#' class='showlogin'> ". __( 'Click here to login', 'woocommerce' )."</a></p>
            </div>";
}
