<?php
define('GOTO_WEBINAR_API_KEY', 'b2bdc6e113540a2d6f9c320a8950a5ba');
class OAuth_En {
...
class OAuth {
    ...
    function getRequiredFields($webinarId) {
        $url = 'https://api.citrixonline.com/G2W/rest/organizers/' . $this->_OAuthEnObj->getOrganizerKey() . '/webinars/' . $webinarId . '/registrants/fields';
        $this->setApiRequestUrl($url);
        $this->setApiRequestType('GET');
        $this->makeApiRequest();
        if ($this->hasApiError()) {
