 ...
 * Tested up to: 3.6.1
 *
 * Text Domain: wc-sequential-order-numbers-pro
...
class WC_Seq_Order_Number_Pro {
	...
	/** Plugin text domain */
	const TEXT_DOMAIN = 'wc-sequential-order-numbers-pro';
	...
	public function __construct() {
		...
		// Installation (must be run *before* the pre_update_option_woocommerce_order_number_start filter is added)
		...
		if ( is_admin() ) {
			...
			add_action( 'woocommerce_settings_start',   array( $this, 'admin_settings_js' ) ); // WC >= 2.0
			add_action( 'woocommerce_settings_tabs',    array( $this, 'admin_settings_js' ) ); // WC 1.6.x
	 ...
	 *        the order is created from the checkout page in the frontend, and
	 *        a post object when the order is created in the admin.  Defaults to
	 *        an array so that other actions can be hooked in
	 */
		...
		foreach ( $settings as $section ) {
			...
			if ( isset( $section['id'] ) && 'general_options' == $section['id'] &&
				...
				$updated_settings[] = array(
					...
					'type'     => 'text',
					'css'      => 'min-width:300px;',
					'default'  => __( 'FREE-', self::TEXT_DOMAIN ), // WC >= 2.0
					'std'      => __( 'FREE-', self::TEXT_DOMAIN ), // WC < 2.0
				);
	...
	public function admin_settings_js() {
		...
		if ( isset( $_REQUEST['tab'] ) && $_REQUEST['tab'] != 'general' ) return;
	...
	public function plugin_manage_link( $actions, $plugin_file, $plugin_data, $context ) {
		...
		return array_merge( array( 'configure' => '<a href="' . admin_url( 'admin.php?page=woocommerce_settings&tab=general' ) . '">' . __( 'Configure', self::TEXT_DOMAIN ) . '</a>' ),
		                    $actions );
	...
	/**
	 * Checks if the free Sequential Order Numbers plugin is active
	 *
	 * @since  1.5
	 * @return bool true if the free Sequential Order Numbers plugin is active, false otherwise
	 */
	public static function is_sequential_order_numbers_active() {
		...
		return in_array( 'woocommerce-sequential-order-numbers/woocommerce-sequential-order-numbers.php', $active_plugins ) || array_key_exists( 'woocommerce-sequential-order-numbers/woocommerce-sequential-order-numbers.php', $active_plugins );
	}
	...
	private function install() {
		...
		$installed_version = get_option( 'wc_sequential_order_numbers_pro_version' );

		if ( false === get_option( 'woocommerce_order_number_start' ) ) {
			// initial installation (can't use wc_sequential_order_numbers_pro_version unfortunately as it's a new option)

			// if the free sequential order numbers plugin is installed and active, deactivate it
			if ( self::is_sequential_order_numbers_active() ) {
		...
		if ( version_compare( $installed_version, self::VERSION ) < 0 ) {

			$this->upgrade( $installed_version );

			// new version number
			update_option( 'wc_sequential_order_numbers_pro_version', self::VERSION );
		}
	}
