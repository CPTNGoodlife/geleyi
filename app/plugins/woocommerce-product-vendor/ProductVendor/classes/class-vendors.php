...
class PV_Vendors
{
		...
		foreach ( $order->get_items() as $key => $product ) {
			...
			if ( $is_vendor ) {
				...
				if ( $group ) {
					...
					$receiver[ $author ][ $product_id ] = array(
						'vendor_id'  => (int) $author,
	...
	/**
	 * Total oustanding commission for a vendor
	 *
	 * @param int $vendor_id
	 *
	 * @return float
	 */
	public static function get_total_due( $vendor_id )
	{
		$total = (float) get_user_meta( $vendor_id, 'pv_total_due', true );
