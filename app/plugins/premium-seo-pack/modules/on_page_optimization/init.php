    ...
    class pspOnPageOptimization
    {
		...
		*/
		private function makePrintBoxParams( $tax=false ) {
			...
			$seo_title = $psp_meta['title'];

			//keyword density
			$__density = $seo->get_keyword_density( $post_content, $focus_kw, false );
			$__nb_words = $__density['nb_words'];
			$__kw_occurences = $__density['kw_occurences'];
			$__density = $__density['density'];
								...
								<table id="psp-seo-score-box" style="width:100%;">
									<tr>
										...
										<td valign="top">
											...
											?>
												<a style="position: relative; bottom: -4px;" id="psp-edit-focus-keywords" class="psp-button blue" href="#edit-focus-keywords">
								...
								<table class="form-table" id="psp-form-meta-tags">
									<tbody>
										...
										<tr>
											...
											<td>
												<textarea name="psp-field-metadesc" id="psp-field-metadesc" rows="3" class="large-text" maxlength="160"><?php echo $psp_meta['description'];?></textarea>
										...
										<tr>
											...
											<td>
												<textarea name="psp-field-metakewords" id="psp-field-metakeywords" rows="3" class="large-text" maxlength="160"><?php echo $psp_meta['keywords'];?></textarea>
											...
											echo $this->uploadImage( 
												array(
												 	'psp-field-facebook-image' => array(
												 		'db_value'	=> $psp_meta['facebook_image'],
							...
							<table class="form-table">
								<tbody>
									<tr>
										...
										<td>
											<select name="psp-field-meta_robots_index" id="psp-field-meta_robots_index">
												<option value="default" <?php echo $psp_meta['robots_index']=='default' ? 'selected="true"' : ''; ?> ><?php _e('Default settings', $this->the_plugin->localizationName);?></option>
												<option value="index" <?php echo $psp_meta['robots_index']=='index' ? 'selected="true"' : ''; ?> ><?php _e('Index', $this->the_plugin->localizationName); ?></option>
												<option value="noindex" <?php echo $psp_meta['robots_index']=='noindex' ? 'selected="true"' : ''; ?> ><?php _e('NO Index', $this->the_plugin->localizationName); ?></option>
											</select>
									...
									<tr>
										...
										<td>
											<select name="psp-field-meta_robots_follow" id="psp-field-meta_robots_follow">
												<option value="default" <?php echo $psp_meta['robots_follow']=='default' ? 'selected="true"' : ''; ?> ><?php _e('Default settings', $this->the_plugin->localizationName);?></option>
												<option value="follow" <?php echo $psp_meta['robots_follow']=='follow' ? 'selected="true"' : ''; ?> ><?php _e('Follow', $this->the_plugin->localizationName); ?></option>
												<option value="nofollow" <?php echo $psp_meta['robots_follow']=='nofollow' ? 'selected="true"' : ''; ?> ><?php _e('NO Follow', $this->the_plugin->localizationName); ?></option>
											</select>
									...
									<tr>
										...
										<td>
											<select name="psp-field-priority-sitemap" id="psp-field-priority-sitemap">
												<option value="-" <?php echo in_array($psp_meta['priority'], array('', '-')) ? 'selected="true"' : ''; ?> ><?php _e('Automatic', $this->the_plugin->localizationName);?></option>
												<?php
												...
												foreach ($__range2 as $kk => $vv){
													...
													echo '<option value="' . ( $vv ) . '" ' . ( $psp_meta['priority'] == $vv ? 'selected="true"' : '' ) . '>' . ( $vv . $__priorityText ) . '</option>';
												}
												?>
											</select>
											<p><?php _e('Should this page be in the XML Sitemap?', $this->the_plugin->localizationName);?></p>
									...
									<tr>
										...
										<td>
											<input type="text" class="large-text" value="<?php echo $psp_meta['canonical']; ?>" name="psp-field-canonical" id="psp-field-canonical">
			...
			<div id="psp-lightbox-overlay">
				<div id="psp-lightbox-container">
					<h1 class="psp-lightbox-headline">
						...
						<span><?php _e('PSP SEO Report for post ID:', $this->the_plugin->localizationName);?> <i></i></span>
	                        	...
	                        	<div class="psp-panel">
	                        		<div class="psp-panel-header">
										<span class="psp-panel-title">
											...
											<?php _e('Mass Optimization', $this->the_plugin->localizationName);?>
										</span>
									</div>
			...
			if( count($post_seo_status) > 0 ) {
				...
				foreach ($post_seo_status as $key => $value) {
					...
					if( $key == 'title' ){
						...
						$html[] = 	'<div class="right-col">';
						$html[] = 		'<p>' . ( $value['debug']['str'] ) . '</p>';
						$html[] = 		'<p><strong>Length:</strong> ' . ( strlen($value['debug']['str']) ) . ' character(s)</p>';
						$html[] = 	'</div>';
						$html[] = 	'<div class="message-box ' . ( $score_html_class ) . '">' . ( $value['msg'] ) . '</div>';
		...
		public function optimize_page( $id="", $kw="" )
		{
			...
			if( !is_null($post) && count($post) > 0 ) {
				...
				if ( $request['action']=='pspOptimizePage' ) { //ajax request from plugin module!
					...
					$__cleanFocusKW = $_REQUEST['psp-field-focuskw']; // clean focus keyword
					
					$post_metas = array_merge($post_metas, array(
						'title'						=> trim( $_REQUEST['psp-field-title'] ),
						'description'				=> trim( $_REQUEST['psp-field-metadesc'] ),
						'keywords'					=> trim( $_REQUEST['psp-field-metakewords'] ),
						'focus_keyword'				=> trim( $__cleanFocusKW ),
						
						'facebook_isactive'			=> trim( $_REQUEST['psp-field-facebook-isactive'] ),
						'facebook_titlu'			=> trim( $_REQUEST['psp-field-facebook-titlu'] ),
						'facebook_desc'				=> trim( $_REQUEST['psp-field-facebook-desc'] ),
						'facebook_image'			=> trim( $_REQUEST['psp-field-facebook-image'] ),
						'facebook_opengraph_type'	=> trim( $_REQUEST['psp-field-facebook-opengraph-type'] ),
						
						'robots_index'				=> trim( $_REQUEST['psp-field-meta_robots_index'] ),
						'robots_follow'				=> trim( $_REQUEST['psp-field-meta_robots_follow'] ),
						
						'priority'					=> trim( $_REQUEST['psp-field-priority-sitemap'] ),
						'canonical'					=> trim( $_REQUEST['psp-field-canonical'] )
					));
