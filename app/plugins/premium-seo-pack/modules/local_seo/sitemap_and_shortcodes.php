			...
			if ( count($value['oh']) > 0 ) {
				...
				foreach ( $value['oh'] as $k => $val ) {
					...
					//$ret[] = '<link itemprop="hasOpeningHoursDayOfWeek" href="http://purl.org/goodrelations/v1#'.$psp_days_list[ "{$val['day']}" ].'" />';
			     	...
			     	// schema.org version!
			     	$ret[] = '<link itemprop="dayOfWeek" href="http://purl.org/goodrelations/v1#'.$psp_days_list[ "{$val['day']}" ].'" />';
			     	$ret[] = '<meta itemprop="opens" content="'.$__from2.'">'.$__from.' - <meta itemprop="closes" content="'.$__to2.'">'.$__to;
				}
					...
					if ( count($value['oh']) > 0 ) {
						...
						foreach ( $value['oh'] as $k => $val ) {
							...
							$__to2 		= $__formated['to_hour'] . ':' . $__formated['to_min'] . ':00';
					     	...
					     	// schema.org version!
					     	$ret[] = '<link itemprop="dayOfWeek" href="http://purl.org/goodrelations/v1#' . ( isset($psp_days_list[ "{$val['day']}" ]) ? $psp_days_list[ "{$val['day']}" ] : '' ) . '" />';
					     	$ret[] = '<meta itemprop="opens" content="'.$__from2.'">'.$__from.' - <meta itemprop="closes" content="'.$__to2.'">'.$__to;
						}
