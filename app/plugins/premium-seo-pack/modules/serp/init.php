	    ...
	    */
		protected function _registerAdminPages()
    	{
    		add_submenu_page(
    			...
    			__('Search Engine Results Page Tracking', $this->the_plugin->localizationName),
	            __('SERP Tracking', $this->the_plugin->localizationName),
	            'manage_options',
			...
			<div id="psp-lightbox-overlay">
				<div id="psp-lightbox-container">
					<h1 class="psp-lightbox-headline">
						...
						<span><?php _e('Your focus keywords list', $this->the_plugin->localizationName);?></span>
	                        	...
	                        	<div class="psp-panel">
	                        		<div class="psp-panel-header">
										<span class="psp-panel-title">
											...
											<?php _e('Search Engine Results Page Reporter', $this->the_plugin->localizationName);?>
										</span>
									</div>
										...
										<form class="psp-form" action="#save_with_ajax">
											<div class="psp-form-row" id="psp-serp-container">
												<div class="psp-top-filters">
													...
													<span id="psp-select-engine-text"><?php _e('Google:', $this->the_plugin->localizationName);?> </span>
													
													<select id="select-engine">
													<option value='--all--'><?php _e('all locations', $this->the_plugin->localizationName);?></option>
													<?php
													...
													<?php /*<input type="button" class="psp-button blue" id="psp-cron-ckeck" value="<?php _e('Cron check now', $this->the_plugin->localizationName);?>">*/ ?>
												...
												<div class="psp-panel psp-serp-filter-keyurl" id="psp-serp-filter-keyurl">
					                        		<div class="psp-panel-header">
														<span class="psp-panel-title">
															<?php _e('Filter Focus Keywords and URLs', $this->the_plugin->localizationName);?>
														</span>
													</div>
														...
														<table class="psp-table" style="border:  none;border-bottom:  1px solid #dadada;width:100%;  border-spacing:0;border-collapse:collapse;">
															<thead>
																<tr>
																	...
																	<th align="left"><?php _e('Focus Keywords', $this->the_plugin->localizationName);?></th>
																</tr>
															</thead>
														 ...
														 <table class="psp-table" style="border: none;border-left: 1px solid #ededed;border-bottom: 1px solid #dadada;width:100%;border-spacing:0;border-collapse:collapse;">
															<thead>
																<tr>
																	...
																	<th align="left"><?php _e('URLs', $this->the_plugin->localizationName);?></th>
																</tr>
															</thead>
												...
												<div class="psp-panel psp-serp-add-keyword" id="psp-serp-add-keyword">
					                        		<div class="psp-panel-header">
														<span class="psp-panel-title">
															...
															<?php _e('Add Keyword <i>(You can keep an eye on your competitors too)</i>', $this->the_plugin->localizationName);?>
															<span id="search-engine-current-loc"><?php _e('Google location currently used: ', $this->the_plugin->localizationName); echo '<strong>'.$this->search_engine."</strong>"; ?></span>
														</span>
													</div>
													<div class="psp-panel-content">
														<div id="psp-add-keyword-block">
															<div style="float: left;width: 450px;">
																<label for="psp-new-keyword"><?php _e('Keyword:', $this->the_plugin->localizationName);?></label>
																<input type="text" id="psp-new-keyword" name="psp-new-keyword" class="psp-new-keyword" value="" />
																<div style="clear:left;"></div>
																<label for="psp-new-keyword-link"><?php _e('URL:', $this->the_plugin->localizationName);?></label>
															...
															<div class="psp-or-block">
																...
																<h2><?php _e('OR', $this->the_plugin->localizationName);?></h2>	
															</div>
															<div style="float: left;width: 300px; ">
																<label><?php _e('Select from your:', $this->the_plugin->localizationName);?></label>
																<div style="clear:left;"></div>
																<input type="button" id="psp-select-fw" class="psp-button gray psp-select-fw" value="<?php _e('Focus keywords', $this->the_plugin->localizationName);?>">
															</div>	
															<div style="clear:left;"></div>
															<input type="button" class="psp-button blue" id="psp-submit-to-reporter" value="<?php _e('Add to Reporter', $this->the_plugin->localizationName);?>">
														</div>
														<div style="clear:left;"></div>
													</div>
												</div>
													...
													pspAjaxListTable::getInstance( $this->the_plugin )
														->setup(array(
															...
															'columns'			=> array(
																...
																'serp_location'	=> array(
																	'th'	=> __('Location', $this->the_plugin->localizationName),
																	'td'	=> '%serp_location%',
																	'align' => 'center',
																	'width' => '80'
																),
																...
																'serp_google'=> array(
																	'th'	=> __('Google Rank', $this->the_plugin->localizationName) . implode("\n", $html_rank_header),
																	'td'	=> '%serp_google%',
																	'align' => 'center',
																	'width' => '120'
																),
																...
																'serp_start_date' => array(
																	'th'	=> __('Start Date', $this->the_plugin->localizationName),
																	'td'	=> '%serp_start_date%',
																	'width' => '115'
																),
		
																'serp_visits'		=> array(
																	'th'	=> __('Visits', $this->the_plugin->localizationName),
																	'td'	=> '%serp_visits%',
																	'width' => '30'
																),
...
<?php
		...
		*/
		public function addToReporter( $keyword='', $link='', $itemid=0 )
		{
			...
			die(json_encode( array(
				'status' => 'valid',
				'data' => $this->get_serp_scores( $request['keyword'], $request['link'], 'default' )
			)));
		}
		...
		public function __getEngineAccessTime()
		{
			...
			if ( isset($_SESSION['psp_engine_access_status']) ) {
				...
				if ( $_SESSION['psp_engine_access_status']=='valid' ) {
					...
					$last_msg .= __('<span class="engine-access-msg-success">' . 'Response received from Google API.' . '</span>', $this->the_plugin->localizationName);
					
				} else {
					$last_msg = '<div class="psp-message psp-error">';
					$last_msg .= __('<span class="engine-access-msg-error">' . 'Could not retrieve response from Google API - you might have used all your available requests for this day!' . '</span>', $this->the_plugin->localizationName);
				}
			}
			...
			die( json_encode(array(
				...
				'last_msg'	=> $last_msg,
				'nb_req'	=> sprintf( __('<span class="engine-access-msg-info">' . 'The number of requests made is <strong>%s</strong> (of maximum %s per day).' . '</span></div>', $this->the_plugin->localizationName), $currentNbReq, $nbReqMax )
			)) );
		}
		...
		*/
		public function __getFocusKW()
		{
			...
			$html[] = 	'<thead>'; 
			$html[] = 		'<tr>'; 
			$html[] = 			'<th>' . __('ID', $this->the_plugin->localizationName) . '</th>'; 
			$html[] = 			'<th>' . __('Focus Keywords', $this->the_plugin->localizationName) . '</th>'; 
			$html[] = 			'<th align="left">' . __('Permalink', $this->the_plugin->localizationName) . '</th>';
			$html[] = 			'<th></th>';  
			$html[] = 		'</tr>'; 
			$html[] = 	'</thead>'; 
			...
			if( count($results) > 0 ){
				foreach ($results as $key => $value){
					...
					$html[] = 	'<td><input type="button" data-itemid="' . ( $value['post_id'] ) . '" data-permalink="' . ( $permalink ) . '" data-keyword="' . ( $value['meta_value'] ) . '" value="' . __('Add to Reporter', $this->the_plugin->localizationName) . '" class="psp-button blue psp-this-select-fw"></td>';
					$html[] = '</tr>';
				}
			}else{
				$html[] = '<tr><td rowspan="3">' . __('No focus keywords for you posts', $this->the_plugin->localizationName) . '</td></tr>';
			}
		...
		*/
		...
		public function cron_add_custom( $schedules ) {
			...
			$schedules['daily'] = array(
				...
				'display' => __('Once Daily', $this->the_plugin->localizationName)
			);
			return $schedules;
		}
		...
		public function cron_reporter_email() {
			...
			foreach ($items as $post){
				...
				if( isset($rank_data) && is_array($rank_data) && count($rank_data) > 0 ){
					...
					$__notInTop100 = __('Not in top 100', $this->the_plugin->localizationName);
			...
			$output = str_replace("{serp_email_title}", __('SERP Keywords Ranking Changes', $this->the_plugin->localizationName), $output);
			
			$output = str_replace("{table_title}", __('Keywords with ranking positions changes on Google since the last rank check on', $this->the_plugin->localizationName) . ' (' . ($items_nr) . ' ' . __('items', $this->the_plugin->localizationName) . ')', $output);

			$output = str_replace("{Focus Keyword}", __('Focus Keyword', $this->the_plugin->localizationName), $output);
			$output = str_replace("{URL}", __('URL', $this->the_plugin->localizationName), $output);
			$output = str_replace("{Google Rank}", __('Google Rank', $this->the_plugin->localizationName), $output);
			$output = str_replace("{Current Rank}", __('Current Rank', $this->the_plugin->localizationName), $output);
			$output = str_replace("{Previous Rank}", __('Previous Rank', $this->the_plugin->localizationName), $output);
			$output = str_replace("{Start Date}", __('Start Date', $this->the_plugin->localizationName), $output);
			$output = str_replace("{Visits}", __('Visits', $this->the_plugin->localizationName), $output);
            ...
            if (isset($this->plugin_settings['cron_email']) && trim($this->plugin_settings['cron_email'])!='') {
            	//$subject = __('Alert | Keywords Ranking Changes | ', $this->the_plugin->localizationName) . str_replace('http://', '', get_bloginfo('url'));
            	$subject = '[' . ( get_bloginfo('name') ) . ']' . __(' Alert | Keywords Ranking Changes | ', $this->the_plugin->localizationName);
            	
            	$headers = array();
            	$headers[] = __('From: '.$this->the_plugin->details['plugin_name'].' SERP module | ', $this->the_plugin->localizationName) . get_bloginfo('name') . " <" . get_bloginfo('admin_email') . ">";
