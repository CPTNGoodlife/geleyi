...
pspFacebookPage = (function ($) {
	...
	function fb_planner_post(atts) {
		...
		var fb_planner_post = {
			...
			autocomplete_fields: function() {
				...
				//if( linkValue != jQuery('#psp_wplannerfb_permalink').val() ) {
				//	jQuery('#psp_wplannerfb_permalink').val(linkValue);
				//}

				/*if( jQuery.trim( jQuery('#psp_wplannerfb_image_upload').val() ) == '' && featuredImg != '' ) {
					jQuery('#psp_wplannerfb_image_upload').val( featuredImg );
					
					var __viewImg = '<a href="#" class="psp_wplannerfb_remove_img button">Remove Image</a><br /><img style="border: 1px solid #dadada;" id="image_psp_wplannerfb_image" src="{plugin_url}/timthumb.php?src={img}&amp;w={thumb_w}&amp;h={thumb_h}&amp;zc={thumb_zc}">';
					__viewImg = __viewImg.replace('{plugin_url}', self.atts.plugin_url );
					__viewImg = __viewImg.replace('{img}', featuredImg );
					__viewImg = __viewImg.replace('{thumb_w}', self.atts.thumb_w );
					__viewImg = __viewImg.replace('{thumb_h}', self.atts.thumb_h );
					__viewImg = __viewImg.replace('{thumb_zc}', self.atts.thumb_zc );

					jQuery('#uploaded_image_psp_wplannerfb_image').html( __viewImg );
					
					// remove image
					$('#uploaded_image_psp_wplannerfb_image').find('a.psp_wplannerfb_remove_img').bind('click',
					function (e) {
						e.preventDefault();
							
						jQuery('#psp_wplannerfb_image_upload').val('');
						jQuery('#uploaded_image_psp_wplannerfb_image').html('');
					});
				}*/
					
				//upload-input-text
				/*if( typeof jQuery('#psp_wplannerfb_upload').find('.upload').val() != 'undefined' ) {
					jQuery.ajax({
						type		: "POST",
						url			: ajaxurl,
						data		: {
							'action'	: 'psp_getFeaturedImage',
							'postId'	: self.atts.post_id
						},
						dataType	: "json",
						success		: function(dataResponse) {
							...
							if( featuredImage.status == 'OK' ) {
								jQuery('#psp_wplannerfb_upload').find('.upload').val(featuredImage.text);
								jQuery('#psp_wplannerfb_upload').find('.screenshot').html('<br /><img src="'+ featuredImage.text +'" width="350" alt=""/><br /><br /><a href="#" class="mlu_remove button">Remove Image</a>');
							}
						},
						error		: function() {
	...
	function fb_scheduler( atts ) {
		...
		jQuery('#psp_wplannerfb_date_hour').live('click', function() {
