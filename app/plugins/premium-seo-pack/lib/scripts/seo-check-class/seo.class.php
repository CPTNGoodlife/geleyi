...
class pspSeoCheck
{
	...
	* @return string
	*/
	public function get_seo_score( $p=0, $kw='', $returnAs='die' )
	{
		...
		if ( $this->the_plugin->__tax_istax( $p ) ) { //taxonomy data!
			...
			$post_content = $this->strip_shortcode($post['post_content']); 
		...
		if( count($post) > 0 ){
			...
			$status['meta_description'] = $this->score_meta_description( $post_metas['description'], $kw );
			$status['meta_keywords'] 	= $this->score_meta_keywords( $post_metas['keywords'], $kw );
	...
	* @return string
	*/
	public function gen_meta_keywords( $str='', $return_nr=10 )
	{
		$str =  $this->strip_shortcode( $str );
		...
		// future, need this list to be added to admin settings
	...
	* @return string
	*/
	public function gen_count_words( $str='' )
	{
		if (empty($str)) return 0;

		$words = $this->gen_meta_keywords( $str, 10000 );
	...
	*/
	public function get_keyword_density( $content, $kw, $single=true )
	{
		...
		$nb_words = 0;
		if ( !empty( $content ) )
			$nb_words = $this->gen_count_words( $content );
	...
	*/
	...
	{
		$str = trim($page_title);
		$str = strtolower($page_title);
		$focus_kw = trim($focus_kw);
		$focus_kw = strtolower($focus_kw);
	...
	function score_meta_description( $page_meta_description, $focus_kw )
	{
		...
		$str = strtolower($page_meta_description);
		$focus_kw = trim($focus_kw);
		$focus_kw = strtolower($focus_kw);
	...
	function score_meta_keywords( $page_meta_keywords, $focus_kw )
	{
		...
		$str = strtolower($page_meta_keywords);
		$focus_kw = trim($focus_kw);
		$focus_kw = strtolower($focus_kw);
	...
	function score_first_paragraph( $page_first_paragraph, $focus_kw )
	{
		...
		$str = strtolower($page_first_paragraph);
		$focus_kw = trim($focus_kw);
		$focus_kw = strtolower($focus_kw);
	...
	function score_enough_words( $page_content, $chars_limit = 250 )
	{
		...
		$words = (int)@str_word_count($page_content);
	...
	function score_images_alt( $page_content, $focus_kw )
	{
		...
		$msgs = array(
			...
			'less' 				=> __( "Bad, your content has %d images and only %d of this contains your most important keywords.", $this->the_plugin->localizationName ),
		...
		if( $total_images > 0 ){
			...
			return array(
				'score' 	=> 0,
				'msg'		=> sprintf($msgs['missing'], $total_images, $kw_images )
			);
		}
	}
	...
	function score_html_bold( $page_content, $focus_kw )
	{
		...
		$msgs = array(
			'less' 				=> __( "Bad, your content has no bold elements.", $this->the_plugin->localizationName ),
			'missing' 			=> __( "Bad, your content has %d bold elements and none of this contains your most important keywords.", $this->the_plugin->localizationName ),
	...
	function score_keyword_density( $density_arr=array() )
	{
		...
		$msgs = array(
			...
			'bad' 		=> __( "Bad, keyword density is %.1f%% and it's not between %.1f%% and %.1f%%, which is the recommended interval. Your content have %d words and the number of keyword occurences in the content is %d.", $this->the_plugin->localizationName ),
			'poor' 		=> __( "Poor, keyword density is %.1f%% and it's not between %.1f%% and %.1f%%, which is the recommended interval. Your content have %d words and the number of keyword occurences in the content is %d.", $this->the_plugin->localizationName ),
			'good'		=> __( "Great, keyword density is %.1f%% and it's between %.1f%% and %.1f%%, which is the recommended interval. Your content have %d words and the number of keyword occurences in the content is %d.", $this->the_plugin->localizationName )
		);
