...
class all_around_visual_elements {
	...
	static public function generate_checkbox ($label, $name, $value, $wrapper=NULL, $style=NULL) {
		...
		$buffer='<div class="fbuilder_checkbox'.$active.'"></div><input class="fbuilder_checkbox_input" name="'.$name.'" id="'.$name.'" style="display:none;" value="'.$value.'" /><div class="fbuilder_checkbox_label"><label for="checkbox_'.$name.'">'.$label.'</label></div><div style="clear:both;"></div>';
		if (is_array($wrapper)) $buffer=self::wrap_it($buffer, $wrapper, '', $name);
		return $buffer;
	}
	...
	static public function generate_color($name, $value, $wrapper=NULL, $style=NULL) {
		$style_string=self::generate_style($style);
		$buffer='<div class="fbuilder_color_wrapper"'.$style_string.'><input class="fbuilder_color fbuilder_input" name="'.$name.'" id="'.$name.'" value="'.$value.'" /><div class="fbuilder_color_display"></div><div class="fbuilder_colorpicker"></div></div>';
		if (is_array($wrapper)) $buffer=self::wrap_it($buffer, $wrapper, '', $name);
		return $buffer;
	}
	...
	static public function generate_button($name, $value, $type, $wrapper=NULL, $style=NULL, $clear=FALSE, $href='', $additional_classes='', $data_value='') {
		$style_string=self::generate_style($style);
		...
		$buffer='<a'.$href.' id="'.$name.'" class="'.$classes.'"'.$style_string.$data_value.'>'.$value.'</a>';
