...
class OptimizePress_PageBuilder {
		...
		if(!isset($this->cur_step)){
			...
			if($this->cur_step > 0){
				...
				$sections = array(
					...
					2 => 'Proceed to Step 3',
					3 => 'Proceed to Step 4',
					4 => 'Save Settings &amp; Launch LiveEditor'
				);
	...
	function print_scripts(){
		if(OP_CONTENT_UPLOAD){
			...
			wp_enqueue_style( OP_SN.'-admin-content-layout',  OP_CSS.'content_layout.css', false, '2011-10-05' );
		} else {
			op_print_scripts(array('page-wizard',$this->cur_step));
			wp_enqueue_style( OP_SN.'-admin-page-wizard',  OP_CSS.'page_builder.css', array(OP_SN.'-admin-common'), '2011-10-05' );
			wp_enqueue_script( OP_SN.'-admin-page-wizard',  OP_JS.'page_builder.js', array(OP_SN.'-admin-common'), '2011-10-05' );
		}
	}
	...
	function setup_wizard(){
		...
		if($this->cur_step > 5){
			return op_show_error('You have completed the wizard.');
		}
	...
	function step_3(){
		...
		if(($dirs = op_dir_list(OP_PAGES.$type)) === false){
			return op_show_error('No themes were found. Please check the themes directory is present and contains themes.');
		}
	...
	function _sections(){
		$sections = array(
			'layout' => array(
				'title' => 'Layout',
				'description' => 'Customize the layout of your template. Setup your headers, navigation bars, feature areas and footers.',
			),
			'color_schemes' => array(
				'title' => 'Colour Scheme',
				'description' => 'Customize the colour scheme for your page. You can use the advanced section to customize individual element colours',
			),
			'functionality' => array(
				'title' => 'On-Page Functionality',
				'description' => 'Control the additional functionality and options for your page below. You can also use the options below to add any custom script code to your page',
			),
		);
		$i = 1;
