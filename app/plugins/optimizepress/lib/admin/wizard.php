...
class OptimizePress_SetupWizard {
	...
	function get_step(){
		if(!isset($this->cur_step)){
			...
			if($this->cur_step > 0){
				$sections = array(
					1 => array('submit_button' => 'Continue to Step 2: Brand Your Blog'),
					2 => array('section' => 'brand','submit_button' => 'Continue to Step 3: Choose Layout'),
					3 => array('section' => 'layout','submit_button' => 'Continue to Step 4: Blog Modules'),
					4 => array('section' => 'modules','submit_button' => 'Continue to Step 5: Finish Setup')
				);
	...
	function print_scripts(){
		...
		wp_enqueue_style(OP_SN.'-admin-common');
		wp_enqueue_script( OP_SN.'-admin-common'); //'-blog-settings');
	}
