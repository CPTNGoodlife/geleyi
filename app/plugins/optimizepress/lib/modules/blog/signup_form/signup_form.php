...
class OptimizePress_Blog_Signup_Form_Module extends OptimizePress_Modules_Base {
	...
	function print_admin_footer_scripts(){
			echo '
<script type="text/javascript" src="'.$this->url.'form.js"></script>';
		$out = array('lang'=>array('add_new_field'=>__('Add New Field',OP_SN),'text'=>__('Text',OP_SN),'title'=>__('Title',OP_SN),'field_name'=>__('Field Name',OP_SN),'remove'=>__('Remove',OP_SN)));
...
var op_mod_signup_form = '.json_encode($out).';
	...
	function print_front_scripts(){
		if(count($this->output_defaults) > 0){
			echo '
<script type="text/javascript" src="'.$this->url.'form_output.js?2011-10-05"></script>';
		}
	}
	...
	function display_settings($section_name,$config=array(),$return=false){
		...
		if(isset($tabs['form_html'])){
			$fields = array('email_data'=>'N','email_address'=>'','redirect_url'=>'','html'=>'','new_window'=>'N',
							'disable_name'=>'N','name_box'=>'','email_box'=>'','method'=>'','action'=>'','extra_fields'=>array(),'email_extra_fields'=>array(),
							'integration_type' => 'custom', 'thank_you_page' => '', 'list' => null, 'action_page' => '', 'gotowebinar' => null);
	...
	function save_settings($section_name,$config=array(),$op,$return=false){
		...
		if(!isset($disabled['form_html'])){
			$fields = array('email_data','email_address','redirect_url','extra_fields','email_extra_fields',
							'html','new_window','disable_name','name_box','email_box','action','fields','method',
							'integration_type', 'list', 'thank_you_page', 'action_page', 'gotowebinar');
			$defaults = array('extra_fields' => array(), 'email_extra_fields' => array(), 'new_window' => 'N', 
							  'disable_name' => 'N', 'fields' => array(), 'method' => 'post',
