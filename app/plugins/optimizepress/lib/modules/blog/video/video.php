...
class OptimizePress_Blog_Video_Module extends OptimizePress_Modules_Base {
	...
	function __construct($config=array()){
		...
		add_action(OP_SN.'-print-footer-scripts',array($this,'print_front_scripts'));
	}

	function print_front_scripts(){
		static $printed;
		if(!isset($printed) && $this->player_count > 0){
			echo '
				<script type="text/javascript" src="'.$this->url.'flowplayer/flowplayer-3.2.6.min.js?2011-10-05"></script>';
		}
	}
	...
	function display_settings($section_name,$config=array(),$return=false){
		...
		$fields = array('type','embed','url','placeholder','hide_controls','auto_play','auto_buffer','width','height',
	...
	function save_settings($section_name,$config=array(),$op,$return=false){
		$video = array('type'=>'url','embed'=>'','url'=>'');
		if(isset($op['type'])){
			$width = 511;
			$height = 288;
			if(isset($config['values'])){
