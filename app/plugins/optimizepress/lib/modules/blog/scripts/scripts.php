				...
				$content .= '
					<li>
						<select name="'.$name.'[position][]">
							<option value="header"'.($header ? ' selected="selected"' : '').'>'.__('Header',OP_SN).'</option>
							<option value="footer"'.(!$header ? ' selected="selected"' : '').'>'.__('Footer',OP_SN).'</option>
							<option value="css"'.(!$header ? ' selected="selected"' : '').'>'.__('Custom CSS',OP_SN).'</option>
						</select>
        ...
        <div class="op-multirow">
            ...
            <?php foreach($items as $item): ?>
				<li>
                    ...
                    <textarea name="<?php echo $name ?>[script][]" cols="40" rows="5"><?php echo stripslashes(op_attr($item['script'])); ?></textarea>
                    <div class="op-multirow-controls">
	...
	function _load_scripts($type){
			...
			if($tmp_scripts = $this->get_option('scripts')){
				...
				foreach($tmp_scripts as $script){
					if(!empty($script['script'])){
						$scripts[$script['position']][] = $script['script'];
					}
				}
			}
		...
		if($type == 'css' && count($scripts[$type]) > 0){
			...
			foreach($scripts[$type] as $css){
				$str .= '<style type="text/css">'.stripslashes($css).'</style>';
			}
