...
class OptimizePress_Modules_Email_Provider_CampaignMonitor implements OptimizePress_Modules_Email_ProviderInterface
{
	...
	{
		if (null === $this->client) {
			$this->client = new CS_REST_General(array('api_key' => $this->apiKey));
		}
	...
	{
		return new CS_REST_Clients($id, array('api_key' => $this->apiKey));
	}
	...
	{
		return new CS_REST_Lists($id, array('api_key' => $this->apiKey));
	}
	...
	{
		return new CS_REST_Subscribers($id, array('api_key' => $this->apiKey));
	}
	...
	{	
		if (isset($data['list']) && isset($data['email'])) {
			...
			if (!$status->was_successful()) {
				...
				wp_die('Error ' . $status->response->Code . ': ' . $status->response->Message);
			}
