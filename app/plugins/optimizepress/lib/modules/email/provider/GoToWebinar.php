...
class OptimizePress_Modules_Email_Provider_GoToWebinar implements OptimizePress_Modules_Email_ProviderInterface
{
	...
	{
		if (null === $this->client) {
			$this->client = new CitrixAPI($this->accessToken, $this->organizerKey);
		}
	...
	public function authorize()
	{
		 ...
		 */
		if (false === op_get('authorize') && false === op_get('disconnect')) {
			/*
			 * Defining callback URL where GoToWebinar will return with auth information
			 */
			$callbackUrl = site_url('/' . OP_GOTOWEBINAR_AUTH_URL);
	...
	public function getData()
	{
		...
		if (is_string($lists)) {
			...
			if (count($lists) > 0) {
				foreach ($lists as $list) {
					$data['lists'][$list->webinarKey] = array('name' => $list->subject, 'fields' => $this->getFormFields());
				}
			}	
		}
