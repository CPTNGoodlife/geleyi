...
class OP_TabbedBlogInfoWidget extends WP_Widget {
	...
	parent::WP_Widget(false, 'OptimizePress: Page Links & Categories Tabs', array('description' => __('Displays tabbed lists of pages, categories and archives from your blog')));
    }
	...
	<p>
            <label for="<?php echo $this->get_field_id('categories')?>">
                Show Categories: <input id="<?php echo $this->get_field_id('categories')?>" name="<?php echo $this->get_field_name('categories')?>" type="checkbox" value="1"<?php echo ($categories==1 ? ' checked' : '')?> />
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('archives')?>">
                Show Archives: <input id="<?php echo $this->get_field_id('archives')?>" name="<?php echo $this->get_field_name('archives')?>" type="checkbox" value="1"<?php echo ($archives==1 ? ' checked' : '')?> />
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('pages')?>">
                Show Pages: <input id="<?php echo $this->get_field_id('pages')?>" name="<?php echo $this->get_field_name('pages')?>" type="checkbox" value="1"<?php echo ($pages==1 ? ' checked' : '')?> />
            </label>
        </p>
        <?php
    }
    ...
    function widget($args, $instance){
        ...
        //Set tabs to show
        $categoriesTab = (isset($instance['categories']) && $instance['categories']==1 ? array('categories' => array(__('Categories',OP_T_SN),'theme'.$theme.'_category_panel',array(array('ulclass'=>'tab-categories miniposts page-list')))) : array());
        $archivesTab = (isset($instance['archives']) && $instance['archives']==1 ? array('archives' => array(__('Archives',OP_T_SN),'theme'.$theme.'_archives_panel',array(array('ulclass'=>'tab-archives miniposts page-list')))) : array());
        $pagesTab = (isset($instance['pages']) && $instance['pages']==1 ? array('pages' => array(__('Pages',OP_T_SN),'theme'.$theme.'_pages_panel',array(array('ulclass'=>'tab-pages miniposts page-list')))) : array());
