...
class OptimizePress_Sections_Email_Marketing_Services {
	...
	function save_icontact($op){
		...
		if ($icontactUsername && $icontactPassword){
	...
	function save_getresponse($op){
		if ($getresponse = op_get_var($op['email_marketing_services'], 'getresponse_api_key')){
			op_update_option('getresponse_api_key', $getresponse);
		}
	}
