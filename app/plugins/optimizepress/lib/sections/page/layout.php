...
class OptimizePress_Sections_Layout {
	...
	function sections(){
		...
		if(!isset($sections)){
			...
			$vals = array('Header &amp; Navigation','Feature Area','Feature Title','Footer Area');
			$type = op_page_option('theme','type');
			if($type == 'launch'){
				array_splice($keys,1,2,array('feature_area','launch_nav'));
				array_splice($vals,1,2,array('Feature Area','Launch Funnel Menu'));
			} elseif($type == 'landing'){
				unset($keys[1],$keys[2],$vals[1],$vals[2]);
				array_splice($keys,0,1,array('header_layout','landing_bg'));
				array_splice($vals,0,1,array('Header &amp; Navigation','Landing Page Background'));
			}
			$section_names = array_combine($keys,$vals);
			$sections = array();
			foreach($section_names as $name => $title){
				if(!(op_page_config('disable','layout',$name) === true)){
					$sections[$name] = array('title' => __($title,OP_SN));
	...
	function save_header_layout($op){
		...
		$fields = array('logo','bgimg','repeatbgimg','bgcolor','link_color');
		$has_error = false;
