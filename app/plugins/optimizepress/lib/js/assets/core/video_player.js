;var op_asset_settings = (function($){
	return {
		...
		customInsert: function(attrs){
			...
			if(type == 'url'){
				content = $.base64.encode(attrs.url || '');
				...
				content = $.base64.encode(attrs.youtube_url || '');
				...
				content = '<img src="'+OptimizePress.imgurl+'video_placeholder.png" alt="'+$.base64.encode(content)+'" width="1" height="1" />';
			}
		...
		customSettings: function(attrs,steps){
			...
			if(type == 'url'){
				$('#op_assets_core_video_player_url').val($.base64.decode(attrs.content || ''));
			} else if (type == 'youtube') {
				$('#op_assets_core_video_player_youtube_url').val($.base64.decode(attrs.content || ''));
			} else {
				$('#op_assets_core_video_player_content').val($.base64.decode($(attrs.content).attr('alt') || ''));
			}
