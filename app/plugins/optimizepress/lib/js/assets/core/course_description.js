	...
	return {
		...
		customInsert: function(attrs){
			var str = '',
				...
				image = (attrs.image || ''),
				title = (attrs.title || ''),
				content = ($.base64.encode(attrs.content) || ''),
				font_str = '';
			...
			str = '[course_description style="' + style + '" icon="' + icon + '" image="' + image + '" title="' + title + '" content="' + content + '"' + font_str+']'+str+'[/course_description]';
		...
		customSettings: function(attrs,steps){
			...
			OP_AB.set_wysiwyg_content('op_assets_core_course_description_content', $.base64.decode(content));
