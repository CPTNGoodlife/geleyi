...
div.field-id-op_assets_core_optin_box_tabs_content_paragraph',
								...
								list: {
									...
									events: {
										change: function(e) {
											...
											// var $email = $('#op_assets_core_optin_box_tabs_form_html_email');
											// $email.empty().append(options);

											//  * Selecting previously selected value (if editing)

											// if (typeof $email.attr('data-default') != 'undefined') {
											// 	$email.val($email.attr('data-default'));
											// }
										}
									}
								},
		...
		customInsert: function(attrs){
			...
			$.each(elems,function(i,v){
				if(v !== null){
					var val = (v=='paragraph' ? ($.base64.encode(attrs.content[v]) || '') : (attrs.content[v] || ''));
		...
		customSettings: function(attrs,steps){
			...
			$.each(val,function(i,v){
				...
				if(v.attrs.name == 'paragraph'){
					...
					if (typeof content != 'undefined') {
						try {
							OP_AB.set_wysiwyg_content(idprefix+'content_paragraph', $.base64.decode(content));
						} catch(e) {
			...
			switch (attrs.integration_type) {
				case 'email':
					...
					vals = {'integration_type': ['integration_type', 'custom'], 'gotowebinar_list': ['gotowebinar', ''], 'email_address':['email_address',''], 'redirect_url':['redirect_url','']};
					break;
				case 'custom':
				case 'oneshoppingcart':
					...
					vals = {'integration_type': ['integration_type', 'custom'], 'gotowebinar_list': ['gotowebinar', ''], 'name':['name_field',''], 'email':['email_field',''], 'method':['method','post'], 'action':['action','']};
					break;
				case 'icontact':
				case 'aweber':
				case 'mailchimp':
				...
				case 'officeautopilot':
					...
					vals = {'integration_type': ['integration_type', 'custom'], 'name':['name_field', ''], 'list': ['list', ''], 'gotowebinar_list': ['gotowebinar', ''], 'thank_you_page': ['thank_you_page', '']};
					break;
			}
			$.each(vals,function(i,v){
				if (i == 'list' || i == 'gotowebinar_list') {
					...
					$('#'+idprefix+'form_html_'+i).val(attrs[v[0]] || v[1]).attr('data-default', attrs[v[0]] || v[1]);
				} else {
			...
			$('#'+idprefix+'form_html_action').val((typeof attrs.action != 'undefined' ? attrs.action : ''));
