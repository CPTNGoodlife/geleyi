;(function($){
	...
	$(document).ready(function(){
		...
		$(document).mousedown( function(e) {
			$('#color-picker').hide().css({'top':'auto','left':'auto'});
	...
	function init_color_pickers(){
		if($('#color-picker').length == 0){
			$('body').append('<div id="color-picker" />');
		}
		var el = $('#color-picker');
		var $t2 = $;
		farbtastic = $.farbtastic('#color-picker',pick_color);
	...
	function init_radio_selectors(){
		$('.img-radio-selector input[type="radio"]').change(function(){
			$(this).closest('.img-radio-selector').find('.img-radio-selected').removeClass('img-radio-selected').end().end().closest('.img-radio-item').addClass('img-radio-selected');
		}).filter(':checked').trigger('change');
		$('.img-radio-label').click(function(){
			$(this).closest('.img-radio-item').find(':radio').attr('checked',true).trigger('change');
		});
		$('.img-radio-selector .preview-option').live('mouseover',function(){
			//$(this).closest('.img-radio-item').find('.img-radio-item-tooltip').fadeIn('fast');
		}).live('mouseout',function(){
			//$(this).closest('.img-radio-item').find('.img-radio-item-tooltip').fadeOut('fast');
		});
	};
	function init_hidden_panels(){
		...
		}).end().find('.show-hide-panel a').live('click',function(e){
			e.preventDefault();
			var $t = $(this),
				func1 = 'addClass',
				func2 = 'show',
