...
add_action('op_pre_tempate_include','op_check_optin_form');
...
function op_optin_box($options,$values,$output,$tpl='',$wrap_elems=array()){
	...
	if(!isset($values['form_open']) || !isset($values['form_close'])){
		$values['form_open'] = '<form action="#">';
		$values['form_close'] = '</form>';
		$values['hidden_elems'] = '';
		$values['email_input'] = '<input type="text" name="email" placeholder="'.$values['content']['email_default'].'" class="email" />';
		if(!$disable_name){
			$values['name_input'] = '<input type="text" name="name" placeholder="'.$values['content']['name_default'].'" class="name" />';
		}
	}
...
function op_post_meta(){
	...
	$comments = sprintf(_n('1 Comment','%1$s Comments',$cn,OP_T_SN), number_format_i18n( $cn ));
	$args = array(__( '
	<p class="post-meta">
		...
		sprintf( esc_attr__( 'View all posts by %s', OP_T_SN), get_the_author() ),
...
function op_init_theme($load_modules=true){
	...
	if($tpl_dir){
		...
		define('OP_T_SN',OP_SN.'_'.$tpl_dir);
		op_textdomain(OP_T_SN,OP_THEME_DIR);
	}
	if($load_modules){
...
function op_init_page_theme($load_modules=true){
	...
	if($tpl_dir){
		...
		define('OP_P_SN',OP_SN.'_p_'.$tpl_dir);
		op_textdomain(OP_P_SN,OP_PAGE_DIR);
...
function op_pagination(array $args=array()){
	...
	if($pages != 1){
		if($paged > 2 && $paged > $range+1 && $showitems < $pages){
			$out[] = '<li class="first-link"><a href="'.get_pagenum_link(1).'">First</a></li>';
		}
		if($paged > 1 && $showitems < $pages){
			$out[] = '<li class="previous-link"><a href="'.get_pagenum_link($paged - 1).'">Previous</a></li>';
		}
		...
		if($paged < $pages && $showitems < $pages){
			$out[] = '<li class="next-link"><a href="'.get_pagenum_link($paged + 1).'">Next</a></li>';
		}
		if($paged < $pages-1 && $paged+$range-1 < $pages && $showitems < $pages){
			$out[] = '<li class="last-link"><a href="'.get_pagenum_link($pages).'">Last</a></li>';
		}
	}
	if(count($out) > 0){
		$out = '
...
<div class="pagination-details cf">
	...
	<p><em>'.sprintf(__('Page %1$s of %2$s', OP_T_SN), $cur, $pages).'</em></p>
</div>';
		if($echo){
			echo $out;
		}
