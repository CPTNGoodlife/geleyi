...
class OptimizePress_Page_Feature_Area_Base {
	...
	function _load_style($name,$options,$feature_settings,$fonts){
		...
		if(defined('OP_LIVEEDITOR')){
			$start = '<div class="op-feature-area" data-unload="0"><div class="op-feature-settings"><a href="#{link_id}" class="feature-settings">Settings</a></div>';
			$end = '</div>';
		}
	...
	function update_feature(){
		...
		$output['output'] = '<div class="op-feature-area" data-unload="0">'.(defined('OP_LIVEEDITOR')?str_replace('{link_id}','feature_area_'.$option,'<div class="op-feature-settings"><a href="#{link_id}" class="feature-settings">Settings</a></div>'):'').$output['output'].op_get_var($options,'after_element').'</div>';
		return $output;	
	}
