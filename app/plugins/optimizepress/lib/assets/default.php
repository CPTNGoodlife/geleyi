...
class OptimizePress_Default_Assets {
		...
		if(defined('OP_PAGEBUILDER')){
			if($GLOBALS['OP_LIVEEDITOR_DEPTH'] == 0){
				...
				if(!preg_match('/'.op_shortcode_regex('op_liveeditor_elements').'/s',$content,$matches)){
					...
					$content = empty($content) ?' ' : ' [op_liveeditor_element]'.$content.'[/op_liveeditor_element] ';
	...
	*/
	function init_shortcodes(){
		...
		add_action('wp_head',array('OptimizePress_Default_Assets','print_css'),12);
	...
	*/
	function _asset_list(){
		$assets_array = array(
			'affiliate_page' => 'Affiliate Page Snippets',
			'arrows' => 'Arrows',
			'audio_player' => 'Audio Player',
			'bullet_block' => 'Bullet Block',
			'button' => 'Button',
			'calendar_date' => 'Calendar Date & Time',
			'content_toggle' => 'Content Toggle',
			'countdown_timer' => 'Countdown Timer',
			// 'countdown_cookie_timer' => 'Countdown Timer with Cookie',
			'course_description' => 'Course Description Box',
			// 'delayed_content' => 'Delayed Content',
			'divider' => 'Divider',
			'dynamic_date' => 'Dynamic Date',
			'feature_block' => 'Feature Block',
			'feature_box' => 'Feature Box',
			'feature_box_creator' => 'Feature Box Creator',
			'file_download' => 'Files Download',
			'guarantee_box' => 'Guarantee Box',
			'headline' => 'Headline',
			'hyperlink' => 'Hyperlink',
			'images' => 'Images',
			'img_alert' => 'Image with Javascript Alert',
			'img_text_aside' => 'Image with Text Aside',
			...
			'optin_box' => 'Optin Box',
			/*'optin_modal' => 'Optin Modal',*/
			'order_box' => 'Order Box',
			'order_step_graphics' => 'Order Step Graphics',
			'pricing_element' => 'Pricing Graphics',
			'pricing_table' => 'Pricing Table',
			'progress_bar' => 'Progress Bar',
			'qna_elements' => 'Q&A Elements',
			'recent_posts' => 'Recent Posts',
			'social_sharing' => 'Social Sharing',
			'step_graphics' => 'Step Graphics',
			'tabs' => 'Tabs',
			'terms_conditions' => 'Terms & Conditions Box',
			'testimonials' => 'Testimonials',
			'tour' => 'Tour Elements',
			'two_column_block' => '2 Column Text',
			'vertical_spacing' => 'Vertical Spacing',
			'video_lightbox' => 'Video Thumbnail & Lightbox',
			'video_player' => 'Video Player'
		);
		// insert the element only if OPM plugin is activated!
		if (defined("WS_PLUGIN__OPTIMIZEMEMBER_VERSION")) {
			$assets_array['membership_order_button'] = 'Membership Order Button';
			$assets_array['membership_download'] = 'Membership Files Download';
	...
	*/
	function credit_card_icons($str,$cc,$align = 'center'){
	...
	function button_2($atts,$content=''){
		...
		if (false === $markup = get_transient('el_' . $uid)) {
			...
			if (!empty($markup)) {
				...
				$markup = self::credit_card_icons(sprintf($markup, do_shortcode(op_clean_shortcode_content($content))), $atts['cc'], $atts['align']);
	...
	*/
	function countdown_timer($atts){
		...
		$data = shortcode_atts(array(
			...
			'years_text_singular' => 'Year',
			'years_text' => 'Years',
			'months_text_singular' => 'Month',
			'months_text' => 'Months',
			'weeks_text_singular' => 'Week',
			'weeks_text' => 'Weeks',
			'days_text_singular' => 'Day',
			'days_text' => 'Days',
			'hours_text_singular' => 'Hour',
			'hours_text' => 'Hours',
			'minutes_text_singular' => 'Minute',
			'minutes_text' => 'Minutes',
			'seconds_text_singular' => 'Second',
	...
	*/
	function course_description($atts){
		...
		$data['img'] = (!empty($image) ? $image : OP_ASSETS_URL.'images/course_description/icons/'.$icon);
		
		//Get image dimensions
		list($data['width'], $data['height']) = getimagesize($data['img']);
	...
	*/
	function feature_block($atts,$content=''){
		...
		$className = ($style=='icon' || $style=='image' ? 'feature-block-with-'.$style : 'feature-block-style-'.$style_number);
		...
		switch($style){
			case 1:
				$html = '
					<li>
						<div>
							<img class="feature-block-1" src="'.OP_ASSETS_URL.'images/feature_block/icons/'.$icon.'" />'
							. $h2_html
			...
			case 2:
				$html = '
					<li>
						<div>
							<span><img class="feature-block-2" src="'.OP_ASSETS_URL.'images/feature_block/icons/'.$icon.'" /></span>'
							. $h2_html
							. op_texturize(do_shortcode(op_clean_shortcode_content($content))).'
						</div>
					</li>
			...
			case 3:
				$html = '
					<li>
						<div>
							<span style="background-color: '.$bg_color.';"><img class="feature-block-3" src="'.OP_ASSETS_URL.'images/feature_block/icons/'.$icon.'" /></span>'
							. $h2_html
							. op_texturize(do_shortcode(op_clean_shortcode_content($content))).'
						</div>
					</li>
			...
			case 4:
				$html = '
					<li>
						<div>
							<span style="background-color: '.$bg_color.';"><img class="feature-block-4" src="'.OP_ASSETS_URL.'images/feature_block/icons/'.$icon.'" /></span>'
							. $h2_html
							. op_texturize(do_shortcode(op_clean_shortcode_content($content))).'
						</div>
					</li>
			...
			case 'image':
				$html = '
					<li>
						<img class="feature-block-image" src="'.$image.'" />
						<div>'
							. $h2_html
							. op_texturize(do_shortcode(op_clean_shortcode_content($content))).'
						</div>
			...
			case 'icon':
				$html = '
					<li>
						<div>
							<img class="feature-block-icon" src="'.OP_ASSETS_URL.'images/feature_block/icons/'.$icon.'" />'
							. $h2_html
							. op_texturize(do_shortcode(op_clean_shortcode_content($content))).'
						</div>
					</li>
	...
	function feature_box($atts,$content=''){
		...
		return '
			<div class="'.$styles[$style].' feature-box-align-'.$alignment.'"'.$block_style.'>
				...
				<div class="feature-box-content"'.$content_style.'>'.$content.'</div>
			</div>
		';
	}
	...
	*/
	...
	function img_text_aside($atts,$content=''){
		...
		if($data['image'] != ''){
			list($data['img_width'], $data['img_height']) = getimagesize($data['image']);
	...
	function live_search($atts)
	{
		...
		$html = '
		<form class="op-live-search-form" action="testaktion" method="get">
			<div class="op-live-search-container">
				...
				<input class="op-live-search-input" name="searchInput" autocomplete="off" placeholder="'.$placeholder.'" />
			</div>
		</form>
		<script>
		...
		';
		wp_enqueue_script('live_search', OP_JS.'live_search.js', array('jquery'), false, true);
		return $html;
	}
	...
	{
		$uidOpBox = 'optbox_' . md5(serialize($atts) . $content);
		...
		if (false === $template = get_transient('el_' . $uidOpBox)) {
			...
			$atts['site_url_process_optin_form'] = site_url('/process-optin-form/');
			$atts['atts'] = $oldAtts;
	...
	function progress_bar($atts,$content=''){
		...
		return $style_tag.'
			...
			<script type="text/javascript">
				jQuery(function() {
					jQuery("#progressbar-'.$id.'").progressbar({ value: 1 });
					jQuery("#progressbar-'.$id.' > .ui-progressbar-value").animate({ width: "'.$percentage.'%"}, 500);
				});
			</script>
		';
	}
...
<li>
	...
	function social_sharing($atts){
		...
		//Init Twitter html
		$data['twitter_html'] = '
			<a href="https://twitter.com/share" class="twitter-share-button"'.(ucfirst($tw_text) != ''?' data-text="'.op_attr($tw_text).'"':'').($tw_url != ''?' data-url="'.op_attr($tw_url).'"':'').' data-lang="'.$tw_lang.'"'.$attrs['tw_extra'].'>'.__('Tweet',OP_SN).'</a>
...
<div class="double-column cf">
		...
		if($data['type'] == 'embed'){
			...
			if(preg_match('|(https?://[^<"]+)|im',$content,$matches)){
				if(strpos($matches[1],'youtube.com') > 0 || strpos($matches[1],'vimeo.com') > 0){
					...
					$data['url'] = OP_MOD_URL.'blog/video/flowplayer/flowplayer-3.2.7.swf?width='.$data['width'].'&amp;height='.$data['height'].'&amp;flashvars=config='.str_replace('"',"'",json_encode($arr));
				}
			}
		}
	...
	function download($atts, $content)
	{
		...
		$html = '';
		$html .= '<li>';
		$html .= '<a href="'. $fileLink.'"><img src="'.OP_ASSETS_URL.'images/membership_download/icons/'.$icon.'" alt="icon" class="thumb" /></a>';
		$html .= '<div class="content">';
		if ($protected) {
			$html .= '<a href="'. $fileLink.'">'.$title.'</a>';
		} else {
			$html .= '<a href="'.$fileLink .'">'.$title.'</a>';
		}
		$html .= '<p>' . $content . '</p>';
	...
	function membership_login_form($atts)
	{
		extract(shortcode_atts(array(
			...
			'link_to_gravatar'			=> '1',
			'display_user_name'			=> '1',
			'my_account'				=> '%%automatic%%',
			'edit_profile'				=> '%%automatic%%',
			'redirection_after_logout' 	=> '%%home%%',
			'additional_code_2'			=> ''
		), $atts));
	...
	function membership_order_button($atts, $content)
	{
		...
		switch ($gateway) {
			...
			case 'google':
				...
				$output = '[optimizeMember-Pro-Google-Button level="'.$level.'" ccaps="' . $ccaps . '" desc="'.$description.'" cc="USD" custom="'.$domain.'" ta="'.$at.'" tp="'.$first.'" tt="'.$days.'" ra="'.$want_to_charge.'" rp="'.$temp[0].'" rt="'.$temp[1].'" rr="'.$temp[2].'" image="default" output="anchor"]' . do_shortcode($content) . '[/optimizeMember-Pro-Google-Button]';
			break;
			case 'paypal':
				$temp = explode('-', $one_time_auth);
				if (empty($first)) {
				...
				$output = '[optimizeMember-PayPal-Button level="'.$level.'" ccaps="' . $ccaps . '" desc="'.$description.'" ps="'.$page_style.'" lc="" cc="USD" dg="0" ns="1" custom="'.$domain.'" ta="'.$at.'" tp="'.$first.'" tt="'.$days.'" ra="'.$want_to_charge.'" rp="'.$temp[0].'" rt="'.$temp[1].'" rr="'.$temp[2].'" rrt="" rra="1" image="default" output="button"]' . do_shortcode($content) . '[/optimizeMember-PayPal-Button]';
			break;
			case 'clickbank':
				...
				$output = '[optimizeMember-Pro-ClickBank-Button cbp="'.$click_product_item.'" level="'.$level.'" ccaps="' . $ccaps . '" desc="'.$description.'" custom="'.$domain.'" tp="'.$temp2[0].'" tt="'.$temp2[1].'" rp="'.$temp[0].'" rt="'.$temp[1].'" rr="'.$rr.'" image="default" output="anchor"]' . do_shortcode($content) . '[/optimizeMember-Pro-ClickBank-Button]';
			break;
		}
		//die($output);
	...
	c.animate({opacity:opac},500);
		...
		if(!defined('OP_LIVEEDITOR')){
			if(self::$add_pretty_photo === true){
				$out .= '
<script type="text/javascript" src="'.OP_JS.'prettyphoto/jquery.prettyPhoto.js"></script>';
			}
		}
...
'.implode("\n",$js).'
	...
	function print_css(){
		echo '
<link href="'.OP_ASSETS_URL.'default.css" type="text/css" rel="stylesheet" media="screen" />
<link href="'.OP_JS.'prettyphoto/prettyPhoto.css" type="text/css" rel="stylesheet" media="screen" />';
	}
	...
	function membership_array($type, $selected_id=0, $parent_id=0) {
		...
		if($rows = $wpdb->get_results($query)){
			foreach($rows as $row){
				...
				$temp[] = '"'. $row->post_parent . '-' . $rowId . '":"' . $title . '"';
			}
		}
