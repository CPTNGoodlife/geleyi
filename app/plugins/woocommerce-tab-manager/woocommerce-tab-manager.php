...
class WC_Tab_Manager {
	...
	/**
	 * Set to true once tabs have been setup.  Used to support the product_page
	 * shortcode in WC < 2.0
	 * @var boolean
	 */
	private $tabs_setup = false;
	...
	public function woocommerce_init() {
		if ( ! is_admin() && ! defined( 'DOING_AJAX' ) ) {
			// setup the tab actions on the frontend
			if ( version_compare( WOOCOMMERCE_VERSION, "2.0" ) >= 0 ) {
				...
				add_action( 'woocommerce_before_single_product', array( $this, 'setup_tabs' ) );
			}
		}
	}
	...
	public function add_plugin_action_links( $actions ) {
		...
		$custom_actions = array(
			...
			'review'    => sprintf( '<a href="%s">%s</a>', 'http://www.skyverge.com/product/woocommerce-tab-manager/#review_form', __( 'Write a Review', self::TEXT_DOMAIN ) ),
		);
	...
	public function setup_tabs() {
		...
		if ( is_product() && ! $this->tabs_setup ) {
			...
			// keep track of the fact that the tabs have been setup
			$this->tabs_setup = true;
		}
	}
